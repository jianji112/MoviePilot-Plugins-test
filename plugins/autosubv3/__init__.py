...
class AutoSubv3(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "自动生成字幕并翻译成中文，支持最新openai sdk，Whisper模型预加载复用，并发翻译优化；自用修改版；"
    # 插件图标
    ...
    _monitor_paths = None
    _lock = Lock()
    _whisper_model = None  # 全局 Whisper 模型实例
    _whisper_model_name = None  # 当前加载的模型名称
    ...
    def __check_asr(self):
        ...
        # 预加载模型
        if self._whisper_model is None or self._whisper_model_name != self._faster_whisper_model:
            logger.info(f"[Whisper] 预加载模型：{self._faster_whisper_model}")
            ...
            self._whisper_model_name = self._faster_whisper_model
            logger.info(f"[Whisper] 模型加载完成，耗时 {time.time() - start:.1f} 秒")
    ...
    def __do_speech_recognition(self, audio_lang, audio_file):
        ...
        try:
            ...
            if self._whisper_model is None:
                logger.error(f"[Whisper] 模型未加载，跳过语音识别")
                return False, None
