...
class AutoSubv3(_PluginBase):
    ...
    def init_plugin(self, config=None):
        ...
        if self._translate_zh:
            ...
            self._batch_size = int(config.get('batch_size')) if config.get('batch_size') else 40
        ...
        try:
            ...
            if self._translate_zh:
                if self._skip_chinese and self.__is_chinese_lang(lang):
                    logger.info(f"检测到中文视频且已开启中文视频不翻译，跳过翻译：{video_file}")
